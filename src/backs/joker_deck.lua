...
local function play_ability(card, context, ability, ret, other_card)
    ...
    if ability.stencil and type(ability.stencil) == "number" and context.full_hand then
        ret = ret or {}
        ret.x_mult = ret.x_mult or 1
        local x_mult = 5 - #context.full_hand
        for _, v in ipairs(context.full_hand) do
            if (v.ability and v.ability.key and v.ability.key == "Joker Stencil") or (v.edition and v.edition.negative) then
                x_mult = x_mult + 1
            end
        end
        ret.x_mult = ret.x_mult + x_mult
        if ret.x_mult <= 1 then ret.x_mult = nil end
    end
